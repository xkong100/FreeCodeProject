...
echo "$($PSQL "SELECT name from games LEFT JOIN teams on games.winner_id = teams.team_id or games.opponent_id=teams.team_id where year = 2014 AND round = 'Eighth-Final' ORDER BY name ASC ")"
